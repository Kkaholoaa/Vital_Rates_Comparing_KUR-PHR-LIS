...
I <- y >= rec.size
delta_size <- y[2] - y[1] #width of cells
...
#testing this out
survival_function <- function(x,s.slp,s.int) {
  #, s.int, s.slp)
  Line <- s.slp * x + s.int
  u <- exp(Line) # From binomial glm, link = logit function
  return(u/(1+u))
}
...
bigmatrix <- function(ModelParameters) {
  ...
  Survival_kernel <- survival_function(y,s.int=s.int, 
                                    s.slp=s.slp)
...
plot(y,Survival_kernel,type = 'l', main = 'Survival - using exp() + u/(1+u)' )
