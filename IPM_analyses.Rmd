...
reproduction_function <- function(y, x, rec, rec.size=rec.size, ind=TRUE) {
  # ind corrsponds to size independent reproduction (a uniform reproduction distribution)
  if (ind == TRUE){
    out <- rep(0,length(x)) 
    out[1:4] <- rec/4} 
...
########## ... Running the IPM ... #############################################
################################################################################
bigmatrix <- function(ModelParameters, RepoType=TRUE) {
  ...
  return(list(K=K, lam=lam, w=stabledist, v=reprodvalue,
              v.dot.w=v.dot.w, sens=sens, elas=elas, G=Growth_kernel, P=P))
}

########################################################################
Scenario="All" # "No2015" # "Yes2015" # 
########################################################################
...
i <- 1
# You need to include the type of repoduction that the model is going to use
# Only two options at the moment w/ TRUE or FALSE 
StableSizeRepo <- FALSE
UniformRepo    <- TRUE
...
pocs       <- bigmatrix(MP_POCS, RepoType=UniformRepo)
pocs_ssd   <- bigmatrix(MP_POCS, RepoType=StableSizeRepo)
...
posp        <- bigmatrix(MP_POSP, RepoType = UniformRepo)  
posp_ssd    <- bigmatrix(MP_POSP, RepoType = StableSizeRepo)  
...
mosp        <- bigmatrix(MP_MOSP, RepoType = UniformRepo)
mosp_ssd    <- bigmatrix(MP_MOSP, RepoType = StableSizeRepo)
...
>>> Devynn changed the bigmatrix function back to what it was (with local variables instead of global) so this next chunk may not run the way that it is currently written. Sorry, Josh!
...
rec.ll <- function(x) {
  ...
  return(-sum(cnt * log(eig.vec), na.rm=TRUE)) 
} # log-likelihood 
...
rec.ss <- function(x) {
  ...
  mod <- bigmatrix()
  eig.vec <- mod$w[II]/sum(mod$w[II])
  return(sum((cnt - eig.vec)^2)) 
} # sums of squares 

pdf("figs/ss_fit_josh.pdf", height=8, width=5)
...
size.dist <- hist(ColonyLevel$log10_SS[ColonyLevel$Genus_Code=="POSP" & ColonyLevel$log10_SS > rec.size & ColonyLevel$log10_SS < max.size], breaks=bin_size, freq=FALSE, border="lightgrey", main="POSP", xlab="Size", ylim=c(0,1))

rec <- recvalpp
mod <- bigmatrix()
lines(y[II], mod$w[II]/sum(mod$w[II]) / delta_size, lty=1)

rec.fit <- mle(rec.ll, start = list(x = 10), method="Brent", lower = 0, upper=1000)
rec.fit <- optimise(rec.ll, c(0, 1000))
rec <- rec.fit$minimum
mod <- bigmatrix()
lam2 <- round(mod$lam, 2)
lines(y[II], mod$w[II]/sum(mod$w[II]) / delta_size, lty=2)

II[1:10] <- FALSE
rec.fit <- optimise(rec.ll, c(0, 1000))
rec <- rec.fit$minimum
mod <- bigmatrix()
lam3 <- round(mod$lam, 2)
lines(y[II], mod$w[II]/sum(mod$w[II]) / delta_size, lty=3)

legend("topright", legend=c("lam=1", paste0("SS fit all (lam=", lam2, ")"), paste0("SS fit trunc (lam=", lam3, ")")), lty=c(1, 2, 3), bty="n", cex=1)
...
size.dist <- hist(ColonyLevel$log10_SS[ColonyLevel$Genus_Code=="POCS" & ColonyLevel$log10_SS > rec.size & ColonyLevel$log10_SS < max.size], breaks=bin_size, freq=FALSE, border="lightgrey", main="POCS", xlab="Size", ylim=c(0,1))
...
size.dist <- hist(ColonyLevel$log10_SS[ColonyLevel$Genus_Code=="MOSP" & ColonyLevel$log10_SS > rec.size & ColonyLevel$log10_SS < max.size], breaks=bin_size, freq=FALSE, border="lightgrey", main="MOSP", xlab="Size")
...
image(y, y, t(pocs$K)^.1, main="Pocillopora Kernel")
...
image(y,y[I],t(pocs$elas[I,])^0.2, xlab="Size (t)",ylab="Size (t+1)",main="Pocillopora Elasticity") #[I] cuts out recruitment
abline(0,1,lwd=3)	# plot 1:1
dev.off()

#Plot Sensitivity
png(filename = paste0("figs/",Name,"_POCS_sensitivity.png"))
image(y,y,t(pocs$sens)^0.2, xlab="Size (t)",ylab="Size (t+1)", main="Pocillopora Sensitivity")
abline(0,1,lwd=3)	# plot 1:1
dev.off()
...
image(y, y, t(posp$K)^.1,main="Porites Kernel")
...
image(y,y[I],t(posp$elas[I,])^0.2, xlab="Size (t)",ylab="Size (t+1)",main="Porites Elasticity") #[I] cuts out recruitment
abline(0,1,lwd=3)	# plot 1:1
dev.off()

#Plot Sensitivity
png(filename = paste0("figs/",Name,"_POSP_sensitivity.png"))
image(y,y,t(posp$sens)^0.2, xlab="Size (t)",ylab="Size (t+1)", main="Porites Sensitivity")
abline(0,1,lwd=3)	# plot 1:1
dev.off()
...
image(y, y, t(mosp$K)^.1,main="Montipora Kernel")
...
image(y,y[I],t(mosp$elas[I,])^0.2, xlab="Size (t)",ylab="Size (t+1)",main="Montipora Elasticity") #[I] cuts out recruitment
abline(0,1,lwd=3)	# plot 1:1
dev.off()

#Plot Sensitivity
png(filename = paste0("figs/",Name,"_MOSP_sensitivity.png"))
image(y,y,t(mosp$sens)^0.2, xlab="Size (t)",ylab="Size (t+1)", main="Montipora Sensitivity")
abline(0,1,lwd=3)	# plot 1:1
dev.off()

#Plot stable size distribution
#png(filename = "figs/AllInt_POCS_stablesizedist.png")
plot(y, posp$w,xlab="Size",type="l",main="POCS Stable size distribution")
...
for (i in 1:nrow(POCS)){ 
  ...
  pocs        <- bigmatrix(MP_POCS, RepoType = Reproduction_Type)
  POCS_lam[i] <- pocs$lam
  K    <- pocs$K
  Sens <- pocs$sens
  Elas <- pocs$elas
...
for (i in 1:nrow(POSP)){
  MP_POSP <- list(rec = recvalpp, 
                  ...
                  s.int = POSP$s.int[i], 
                  s.slp = POSP$s.slp[i])
  posp        <- bigmatrix(MP_POSP, RepoType = Reproduction_Type)
  POSP_lam[i] <- posp$lam
  K    <- posp$K
  Sens <- posp$sens
  Elas <- posp$elas
...
for (i in 1:nrow(MOSP)){
  ...
  mosp        <- bigmatrix(MP_MOSP, RepoType = Reproduction_Type)
  MOSP_lam[i] <- mosp$lam
  K    <- mosp$K
  Sens <- mosp$sens
  Elas <- mosp$elas
...
AllD$EndYear[AllD$Years=="All Years"]=2019.5
AllD$StartYear=2000+as.numeric(substr(AllD$Years,1,2))
AllD$StartYear[AllD$Years=="All Years"]=2012.5
AllD$Island=factor(AllD$Island,levels=c("All","HAW","MAI","OAH","FFS","KUR"))
GeneraLU=c("Porites sp.","Monitpora sp.","Pocillopora sp.")
...
LambdaBySite=ggplot(subset(AllD,Sites!="All Sites"),aes(x=EndYear,y=log2(Lambdas),size=GModel.n,fill=log2(Lambdas)))+
  ...
  facet_wrap(Genus~Sites)+
  scale_color_manual(name="Site",values = mycolors)+
  scale_fill_gradient2(name="Lambda",midpoint = 0,breaks = log2(seq(.8,1.5,by=.1)),labels = seq(.8,1.5,by=.1))+
  scale_size_binned(name="N Colonies",trans = "log10")+
  scale_y_continuous(breaks = log2(seq(.8,1.5,by=.05)),labels = seq(.8,1.5,by=.05),name="Lambda - Const. Region Recruitment")+
  scale_shape_manual(name="Island",values = c(20:25))+
  theme_bw()
LambdaBySite
ggsave(filename = "./figs/LambdasBySitePlots.jpg",plot = LambdaBySite,width=8,height=8)
...
names(AllD)
demo=AllD[,c("Sites","Years","StartYear","EndYear","Genus","g.int","g.slp","g.var","s.int","s.slp","Lambdas")]
library(vegan)
ORD=decorana(demo)
