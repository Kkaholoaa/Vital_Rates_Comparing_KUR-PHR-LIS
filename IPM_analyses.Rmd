...
ggplot(AllD,aes(x=R.sqd,y=pR2,size=g,fill=log2(Lambdas)))+
...
ggplot(AllD,aes(group=Genus_Code))+
  geom_hline(yintercept = c(14),size=3,color="gold")+
  geom_hline(yintercept = c(15),size=3,color="red")+
  geom_hline(yintercept = c(19),size=3,color="orange")+
  ...
  geom_point(aes(shape=Genus_Code,y=StartYear,x=Sites),fill="black",size=1,position=position_dodge(width=.5),color="black",alpha=1)+#,data=subset(AllD,Genus_Code=="POCS"))+
  geom_linerange(aes(ymin=StartYear,ymax=EndYear,x=Sites,color=log2(Lambdas)),size=2,position=position_dodge(width=.5),alpha=1)+#,data=subset(AllD,Genus_Code=="POCS"))+
  geom_vline(xintercept = c(1.5,2.5,3.5,5.5,7.5,11.5))+
  scale_fill_gradient2(name="Lambda",midpoint = 0)+
  scale_color_gradient2(name="Lambda",midpoint = 0)+
  scale_shape_manual(values = c(22,21,23))+
#  scale_size_area(limits=log2(c(.5,2)),breaks=log2(seq(.5,2,length.out = 5)),labels=seq(.5,2,length.out = 5),oob=squish)+
  coord_equal()+
  coord_flip()+
  theme_bw()+
  theme(axis.text.x = element_text(angle=90,size = 10,hjust = 0),panel.background=element_rect(fill = "gray50"))


ggplot(AllD,aes(x=log2(Lambdas)))+
...
recvalpc <-  28.55 #21.23
recvalpp <-  24.85 #20.8
recvalmp <-  25.25#20.94
