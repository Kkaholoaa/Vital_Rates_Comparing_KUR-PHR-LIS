...
```{r}
#TO DO
#need to log transform TransitionRate_L and StartingSize? then re run my fit model ^
#need to remove the NAs from the TransitionRate_L column
...
plot(ln_ES ~ ln_SS ,data = growth_POCS ,xlim=c(-3,10), ylim=c(-3,10), main="All Pocillopora growth at all sites" )  #the final index of the ln_ES aka the largest size
abline(fit_POCS)
abline(a=0, b=1,col="red")
...
#y = 0.8462x + 1.0236
summary(fit_POSP)
plot(ln_ES ~ ln_SS ,data = growth_POSP ,xlim=c(-3,10), ylim=c(-3,10) , main="All Porites growth at all sites")  #the final index of the ln_ES aka the largest size
abline(fit_POSP)
abline(a=0, b=1,col="red")
#smaller sizes grow faster and the larger sizes are right at the 1:1
...
plot(ln_ES ~ ln_SS ,data = growth_MOSP ,xlim=c(-3,10), ylim=c(-3,10), main="All Montipora growth at all sites",
     legend("topleft", legend = c("Line1","Line2")) )  #the final index of the ln_ES aka the largest size
abline(fit_MOSP)
abline(a=0, b=1,col="red")
#need to plot linear model
ggplot(growth_MOSP, aes(x=ln_SS, y=ln_ES))+ geom_point() + geom_abline()
#smaller sizes grow faster and the larger sizes are right at the 1:1
...
plot(ln_ES ~ ln_SS ,data = growth_MAISIOK01 ,xlim=c(-3,10), ylim=c(-3,10))  #the final index of the ln_ES aka the largest size
