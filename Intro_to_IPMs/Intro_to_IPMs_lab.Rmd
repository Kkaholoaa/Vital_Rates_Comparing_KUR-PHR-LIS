...
YOOOOO ADD THIS 1:1 LINE
Use this to add the 1:1 Lin in the other plots:
...
##A. Plots for Data exploration
Read in data, make some plots
```{r}
# A. plots for data exploration


# read in data. the data is a .csv file included in the same folder as this file
d=read.csv('/Users/c-rod/Documents/GitHub/VitalRates/Intro_to_IPMs/Intro_to_IPMs_Exercises_Data.csv')
#View(d)
...
head(d)
	# and that new recruits are stored at the end and were only observed in the second survey (2002). To differentiate b/w new recruits and flowers that survived, leave survival, seeds, and repro blank
tail(d)
...
plot(d$size,d$fec.seed,main='Seeds') # jittered to see easier, # of seeds produced dep. on parent size
hist(d$sizeNext[is.na(d$size)],main='Size of Recruits') #wherever size is NA, plot AKA the recruits. So it plots the frequency of recruit size to assess the size distribution of the babies
...
Build regressions for survival, growth, seeds, size distribution of recruits, and establishment probability. Plot the regressions
```{r}
# B. regressions build regressions for vital rate functions
...
surv.reg=glm(surv~size,data=d,family=binomial()) #generalized linear model AKA linear regression
summary(surv.reg) #lower the AIC, the better the model fit?
params$surv.int=coefficients(surv.reg)[1] #survival model =surv.reg. First coefficient is intercept
params$surv.slope=coefficients(surv.reg)[2] #second coefficient is slope
#print(coefficients(surv.reg)[2]) 

# 2. growth regression
growth.reg=lm(sizeNext~size,data=d) #linear model
summary(growth.reg)
...
seed.reg=glm(fec.seed~size,data=d,family=poisson()) #generalized linear model
summary(seed.reg)
...
# 5. establishment probability
	# these data represent a single year's worth of data, hence establishment probability can be estimated by dividing the number of observed recruits by the number of seeds. 
#hence the growth/survival measurements were taken in year t which the recruit sizes were measured in year t+1.
params$establishment.prob=sum(is.na(d$size))/sum(d$fec.seed,na.rm=TRUE) # number recruits/number seeds
...
par(mfrow=c(2,2),mar=c(4,4,2,1))
xx=seq(0,8,by=.01)
plot(d$size,d$sizeNext,main='Growth/Shrinkage/Stasis')	#plot change in size
	lines(xx,predict(growth.reg,data.frame(size=xx)),col='red',lwd=3)
	abline(0,1,lwd=3)	# plot 1:1, which represents stasis
plot(d$size,jitter(d$surv),main='Survival') # Survival plot. jittered to see easier
	lines(xx,predict(surv.reg,data.frame(size=xx),type='response'), col='red',lwd=3)
plot(d$size,d$fec.seed,main='Seeds') # jittered to see easier. Size of parent and the # of seeds produced
	lines(xx,predict(seed.reg,data.frame(size=xx),type='response'), col='red',lwd=3)
#Plot recruit density histogram
...
# C. build vital rate functions
Build survival probability, growth, and reproduction functions
```{r}
...
# these functions represent the modeler's decision about how to decompose the life cycle. in this very simple example, we model (1) survival, (2) growth, (3) seed number, (4) the seedling size distribution and (5) the establishment probability. 
#these last three functions are combined in the model for fecundity below. 
#in practice, we'd need to decide what regressions to build in part B in advance, but it's easier to digest this section after you've seen the regressions above, so fortunately we built all the right regressions already. in practice, sections B and C are iterative; one might inform one another. 


## vital rate functions

# 1. probability of surviving
#red line in survival plot (above) is a logistic regression. That red line has a slope (surv.slope) and intercept (surv.int)
#function u/1+u is some type of logistic function
#u is the best fit line
...
g.yx=function(xp,x,params) { 			
	dnorm(xp,mean=params$growth.int+params$growth.slope*x,sd=params$growth.sd) #variance = standard deviation, growth.int = intercept
}
#makes a normal distribution (dnorm) w/ the intercept , slope, and SD
length(params$growth.int)
print(params$growth.sd)
...
image(y,y,t(G),main='growth kernel') 		# plot growth kernel
abline(0,1,lwd=3)	# plot 1:1, which represents stasis

S=s.x(y,params=params) 							# survival 
plot(y,S,type='l',main='survival')	# plot survival kernel
...
F=h*outer(y,y,f.yx,params=params) 	# fecundity (reproduction) kernel
image(y,y,t(F),main='fecundity kernel')	# plot fecundity kernel
...
image(y,y,t(K)^.3,main='full kernel^0.3')			# plot full leveled out kernel
abline(0,1,lwd=3)	# plot 1:1, which represents stasis
...
w.eigen=Re(eigen(K)$vectors[,1]) #right eigen
stable.dist=w.eigen/sum(w.eigen)  #use right eigenvector gives you stable size dist
v.eigen=Re(eigen(t(K))$vectors[,1]) #left eigenvector
repro.val=v.eigen/v.eigen[1] #gives the reproductive value
...
```
Kernel: Colors signify the probability that a plant will transition to a size class. Greatest recruitment is from largest plants Positive growth for medium plants, negative growth for large plants
Sensitivity: darker red means more sensitive. Pop growth is very sensitive to rapid growth of small plants and growth of medium-large plants affects population growth
Elasticity: darker red means more probable. Pop growth depends on slight growth/stasis of medium-large plants. Recruitment does not have a big impact on pop growth
