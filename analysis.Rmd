...
Change <- MOSP_G #change this variable rather than having to change it everywhere
these  <- grepl('\n',Change$Sites)
Change[these,] <- separate(Change[these,], col=Sites, 
                          into = c("Sites","Years"), sep = "\n" )

MOSP_G <- Change #change this variable rather than having to change it everywhere
...
ggplot(GPlotz, aes(x=Sites, y=Years)) + 
  geom_raster(aes(fill = g.slp))+ #change fill to look at diff VR functions
  scale_fill_gradient2(low = "red", high = "blue", mid="white",midpoint = GPlotz$g.slp[1],na.value = "grey" )+
  labs(title = sprintf("Growth matrix visualization: %s", GPlotz$Genus_Code[1])) + theme_dark() +
...
MP_MOSP <- list(rec = totrecruits_MOSP, 
                g.int = MOSP_g$g.int[1], 
                g.slp = MOSP_g$g.slp[1], 
                g.var = 0.035, 
                s.int = MOSP_s$s.int[1], 
...
image(y, y, t(pocs$K)^.1,)
title(GeneraNames[1], line=-1)
abline(0, 1, lty=2)
legend("topleft", bty = "n", lty = c(0,2), col = 1,
       legend= c(sprintf("lambda = %.2f", round(pocs$lam,2)), "1:1 line"))
dev.off()
...
abline(0, 1, lty=2)
legend("topleft", bty = "n", lty = c(0,2), col = 1,
       legend= c(sprintf("lambda = %.2f", round(posp$lam,2)), "1:1 line"))
dev.off()
...
abline(0, 1, lty=2)
legend("topleft", bty = "n", lty = c(0,2), col = 1,
       legend= c(sprintf("lambda = %.2f", round(mosp$lam,2)), "1:1 line"))
dev.off()
