...
AllDataFrame <- load("data/Patch_And_Colony_Data_20210531.rdata")
...
survivalPOCS <- subset(surv_dat, Genus_Code == "POCS")
survivalPOSP <- subset(surv_dat, Genus_Code == "POSP")
survivalMOSP <- subset(surv_dat, Genus_Code == "MOSP")
auc(survivalPOCS$survival, survivalPOCS$size)
auc(survivalPOSP$survival, survivalPOSP$size)
auc(survivalMOSP$survival, survivalMOSP$size)
...
```{r PlotGrowthMatrix,}
#Growth
GPlotz <- POCS_G #change this variable

plotz<- ggplot(GPlotz, aes(x=Sites, y=Years)) + 
  geom_raster(aes(fill = g.int))+ #change fill to look at diff VR functions
  scale_fill_gradient2(low = "red", high = "blue", mid="white",midpoint = GPlotz$g.int[1],na.value = "grey" )+
...
plotz <- ggplot(SPlotz, aes(x=Sites, y=Years)) + 
  ...
  scale_fill_gradient2(low = "red", high = "blue", mid="white",midpoint = SPlotz$s.int[1],na.value = "grey" )+
...
#all of the IPM analysis needs to be moved into a new .rmd file
```{r IPM_TESTING,}
Growth <- subset(ColonyLevel, TransitionTypeSimple == "GROWTH" | 
                   TransitionTypeSimple == "SHRINK")
...
bin_size <- min.size + c(0:50) * (max.size - min.size)/50 #n aka mesh size = 50
