...
Change <- POCS_G #change this variable rather than having to change it everywhere
these  <- grepl('\n',Change$Sites)
...
plotz <- ggplot(GPlotz, aes(x=Sites, y=Years)) + #when I get ss and tt working, change the x= and y-
  geom_raster(aes(fill = g.slp))+ #change fill to look at diff VR functions
  scale_fill_gradient(low = "grey90", high = "purple" )+
  labs(title = sprintf("Growth matrix visualization: %s", GPlotz$Genus_Code[1])) + theme_bw() +
