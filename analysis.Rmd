...
# Survival model 
    if (length(subset3_S$size) >= minTrans){
      ...
      nullmodS <- glm(survival ~ 1, family = "binomial", data = subset1_S)
      pR2[y]   <- 1 - logLik(modS) / logLik(nullmodS)
...
for (i in 1:length(Focus)){
  ...
  nullmodS  <- glm(survival ~ 1, family = "binomial", data = subset1_S)
  pR2       <- 1 - logLik(modS) / logLik(nullmodS)
  ...
  dfS <- data.frame(Sites = "All Sites", Years = "All Years", Genus_Code = Focus[i],
                                    ...
                                    AIC = aic_S, pR2=pR2, SModel.n = Snote) 
