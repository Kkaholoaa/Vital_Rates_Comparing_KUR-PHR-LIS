...
DO NOT NEED ANYMORE (CAN DELETE SOON!!)
...
for (g in 1:TNam){
  ...
  b <- a + YNam -1
  rng <- a:b
  UniqueIDs[rng] <-sprintf("%s_%s_All_Sites", 
                           rep(GeneraNames[g],length = YNam), 
                           RIntY)}
for (s in 1:SNam){
  a <- b+1
  b <- a + YNam -1
  rng <- a:b
  UniqueIDs[rng] <-sprintf("%s_%s_All_Genera", 
                           rep(SiteNames[s],length = YNam), 
                           RIntY)}
...
#Survival Model
Survival <- 1 - ColonyLevel$Mortality
modS <- glm(Survival ~ ln_SS, family = "binomial", data = ColonyLevel)

# saving model coefficients and notes into dataframe with corresponding uniqueID
GrowthDataFrame <- data.frame(Model_IDs ="All_Genera_Site_Years", g.int = coef(mod)[1], 
                              g.slp = coef(mod)[2], growth_notes = 
                                sprintf("Num of transitions used equals %s", 
                                        length(Growth$ln_SS)))

SurvivalDataFrame <- data.frame(Model_IDs = "All_Genera_Site_Years", s.int = coef(modS)[1], 
                              s.slp = coef(modS)[2], survival_notes = 
                                sprintf("Num of transitions used equals %s", 
...
for (i in 1:LoopLength){
  ...
  df <- data.frame(Model_IDs = sprintf("%s_All_Sites_Years",Focus[i]), 
...
for (i in 1:LoopLength){
  ...
  df <- data.frame(Model_IDs = sprintf("%s_All_Taxa_Years",Focus[i]), 
                 g.int = growth[1,i], g.slp = growth[2,i], 
                 growth_notes = note[i])
...
for (i in 1:LoopLength){
  ...
  df <- data.frame(Model_IDs = sprintf("%s_All_Taxa_Sites",Focus[i]), 
                 g.int = growth[1,i], g.slp = growth[2,i], 
                 growth_notes = note[i])
...
Spec_selection <- ColonyLevel$Genus_Code
Spec_selectionSF <- ColonyLevel$Site
...
for (i in 1:LoopLength){
  ...
  for (v in 1:LoopLengthSF){
    ...
    if (length(ln_SS) > minTrans){
      mod <- lm(ln_ES ~ ln_SS)
      note[v] <- sprintf("Num of transitions = %s", 
                      length(ln_SS))
      growth[,v] <- coef(mod)}
...
[ ] Add AIC calcs into the dataframe
[ ] Add variance into the dataframe
