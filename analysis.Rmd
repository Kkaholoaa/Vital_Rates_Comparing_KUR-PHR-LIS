...
GrowthDataFrame <- data.frame(Model_IDs ="All_Genera_Site_Years", 
                              g.int = coef(mod)[1], 
...
SurvivalDataFrame <- data.frame(Model_IDs = "All_Genera_Site_Years", 
                                s.int = coef(modS)[1], s.slp = coef(modS)[2], 
                                AIC = aicValS, survival_notes = 
                                sprintf("Num of transitions used equals %s", 
                                        length(ColonyLevel$ln_SS)))

plot(jitter(Survival, 0.1) ~ ln_SS, ColonyLevel)
ss <- seq(-3, 9, 0.1)
lines(ss, predict(modS, list(ln_SS = ss), type="response"), pch=8)
```
...
Spec_selection <- ColonyLevel$Genus_Code
Spec_selectionSF <- ColonyLevel$Site
Spec_selectionSF2 <- ColonyLevel$Interval_Years
...
1.b. Is this the best way to calculate the growth variance?
