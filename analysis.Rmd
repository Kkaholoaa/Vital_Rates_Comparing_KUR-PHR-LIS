...
ColonyLevel$ln_ESc[ColonyLevel$ln_ESc == -Inf] <-NA
...
for (i in 1:LoopLength){
  ...
  for (v in 1:LoopLengthSF){
    ...
    for (y in 1:LoopLengthSF2){
      ...
      df <- data.frame(Sites = subFocus[v], Years = subFocus2[y], 
...
save(GrowthDataFrame, file = "data/SiteInterval_Gmodfits.rdata")
```
...
# Survival model 
    if (length(subset3_S$size) >= minTrans){
      ...
      #rocS     <- roc(data = subset3_S, predictor = size, response = survival)
    ...
    dfS <- data.frame(Sites = subFocus[v], Years = subFocus2[y], Genus_Code = Focus[i],
                                  s.int = survival[1,y], s.slp = survival[2,y], 
                                  AIC = aic_S[y], 
                                  SModel.n = Snote[y]) #AUC=AUC[y],
...
for (i in 1:length(Focus)){
  ...
  #rocS     <- roc(data = subset3_S, predictor = size, response = survival)
  Snote <-  length(subset3_S$size)
  
  survival <- coef(modS)
  aic_S    <- AIC(modS)
  dfS <- data.frame(Sites = "All Sites", Years = "All Years", Genus_Code = Focus[i],
                                    s.int = survival[1], s.slp = survival[2], 
                                    AIC = aic_S, SModel.n = Snote[y]) #AUC=AUC[y],
      
  SurvivalDataFrame <- rbind(SurvivalDataFrame,dfS)
}

save(SurvivalDataFrame, file = "data/SiteInterval_Smodfits.rdata")

combo <- cbind(GrowthDataFrame,SurvivalDataFrame[4:7])
save(combo, file = "data/SiteInterval_allmodfits.rdata")

no_NAs <- na.omit(combo)
save(no_NAs, file = "data/SiteInterval_allmodfits_noNAs.rdata")
```
