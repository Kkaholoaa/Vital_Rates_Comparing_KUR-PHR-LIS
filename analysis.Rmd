...
```{r}
Choice <- MOSP # Change this to the Genera Name above that you care about
...
dt <- rbind(dt, data.frame(ID_x=rep('All',length = 2),
                           ID_y=rep('All',length = 2),
                           Genus_Code=rep(Choice$Genus_Code[1],2),
                           g.int=coef(mod)[1],
                           g.slp=coef(mod)[2]))
...
dt <- rbind(dt, data.frame(ID_x=x2_Y,ID_y=rep('All_Sites',length(x2_Y)),
...
# Saving parameters & model summaries into dataframes
dS <- rbind(dS, data.frame(Sites=rep('All',2),Years=rep('All',2),
                           Genus_Code=rep(Choice$Genus_Code[1],2),
                           s.int=coef(modS)[1],
                           s.slp=coef(modS)[2]))
...
growing <- read.csv("growthmodel.csv")
...
ggsave(filename = "Figures/POSP_growthmatrix_slp.png", plot = plotz)
...
ggplot(dr, aes(x=Sites, y=Times )) + #change for size depend vs independ
  geom_raster(aes(fill = Tots_recs_peryear ))+ 
  scale_fill_gradient(low = "grey90", high = "purple" )+
  labs(title = "Size indepedent matrix visualization example") + theme_bw() +
  theme(plot.title = element_text(size = 20, face = "bold"), axis.text.x =  element_text(angle = 45, hjust = 1)) 
  #xlab(" ") + ylab(" ")+
