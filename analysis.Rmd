...
#AllDataFrame <- load("data/Patch_And_Colony_Data_20210402.rdata")
...
for (i in 1:length(GeneraNames)){
  ...
  #Then loop by site. Want a bar for each site and timepoint. Then avg the timepoints to get 1 bar per site
  #initialize vector
  ...
  for (l in 1:length(SiteNames)) {
    ...
    #Want a bar for each time interval
     #calculate unique time intervals
     timeint_ <- unique(subsetz2$Interval)
     
     #calc # of recruits for each time interval
     numbrecruits <- rep(0,length(timeint_))
     for (w in 1:length(IntervalYears)){
       numbrecruits[w] <- sum(subsetz2$Recruit[which(subsetz2$Interval ==
                                              timeint_[w])])
     }
     #avg timepoints (per site) to get std dev
    
  cat<- data.frame(ID=sprintf("%s_%s",GeneraNames[i],SiteNames[l]), Mean= mean(numbrecruits), StdError= sd(numbrecruits)/sqrt(length(numbrecruits)) )
