...
#### SITE ######################################################################
################################################################################
#making interval bins instead of using the 0.92343 IntervalYears 
Growth$IntervalName <- paste0(Growth$StartingYear,"_",Growth$EndingYear)

mod_site_POCS <- lm(ln_ESc ~ ln_SS + Site:ln_SS + IntervalName:ln_SS, 
           data = Growth[Growth$Genus_Code == GeneraNames[1],])
summary(mod_site_POCS)
drop1(mod_site_POCS, test="F")

mod_site_POSP <- lm(ln_ESc ~ ln_SS + Site:ln_SS + IntervalName:ln_SS, 
           data = Growth[Growth$Genus_Code == GeneraNames[2],])
summary(mod_site_POSP)
drop1(mod_site_POSP, test="F")

mod_site_MOSC <- lm(ln_ESc ~ ln_SS + Site, 
...
plot(ln_ES ~ ln_SS ,data = Growth, main="Full growth fit")
...
plot(ln_ES ~ ln_SS ,data = Growth, main="Genera growth fit at all sites+ all interval years")
...
par(mfrow=c(4,3))
for (h in 1:LoopLeng) {
  plot(ln_ES ~ ln_SS ,data = Growth, main="Each site growth fit for all genera + all interval years")
