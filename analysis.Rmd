...
```{r}
Choice <- POCS # Change this to the Genera Name above that you care about
...
for (i in 1:length(Ints)){
  if (length(Choice$Interval == Ints[i]) <= Data_Min){
    ...
    a_recs    <- Choice$Recruit[which(Choice$Interval == Ints[i])]
    times     <- round(Choice$Interval_Years[which(Choice$Interval == Ints[i])])
    areas     <- round(Choice$StartingSize[which(Choice$Interval == Ints[i])])
    #a_recs   <- a_recs/areas
    recs_I[i] <- round(sum(a_recs/times),2)
...
Site_surv <- c()
Site_grow <- c()
ggplot(POCS_S, aes(x=Sites, y=Years))+
  geom_raster(aes(fill = s.int))+
