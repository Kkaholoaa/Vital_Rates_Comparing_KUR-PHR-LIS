...
Change <- POSP_Rdep #change this variable rather than having to change it everywhere
these  <- grepl('\n',Change$Sites)
Change[these,] <- separate(Change[these,], col=Sites, 
                          into = c("Sites","Years"), sep = "\n" )

POSP_Rdep <- Change #change this variable rather than having to change it everywhere

```

```{r}
GPlotz <- MOSP_G #change this variable
...
ggsave(filename = sprintf("Figures/%s_growthmatrix_%s.png",
                  GPlotz$Genus_Code[1],"slp"), plot = plotz)


#recruitment
RPlotz <- POSP_Rdep #change this variable
...
SPlotz <- POCS_S #change this variable
plotz <- ggplot(SPlotz, aes(x=Sites, y=Years)) + 
  geom_raster(aes(fill = s.int))+ #change fill to look at diff VR functions
  scale_fill_gradient(low = "grey90", high = "purple" )+
  labs(title = sprintf("Survival matrix visualization: %s", SPlotz$Genus_Code[1])) + theme_bw() +
  theme(plot.title = element_text(size = 20, face = "bold"), axis.text.x =  element_text(angle = 45, hjust = 1)) 
...
RPlotz <- POCS_R #change this variable
...
POCS_R <- subset(POCS_R,
                   Years == "All_Years" )
totrecruits_POCS <- sum(POCS_R$Tots_recs_peryear)
POSP_R <- subset(POSP_R,
                   Years == "All_Years" )
totrecruits_POSP <- sum(POSP_R$Tots_recs_peryear)
MOSP_R <- subset(MOSP_R,
                   Years == "All_Years" )
totrecruits_MOSP <- sum(MOSP_R$Tots_recs_peryear)

#run chunk below to get the functions
#NEXT

MP_POCS <- list(rec = totrecruits_POCS, g.int = POCS_g$g.int[1], g.slp = POCS_g$g.slp[1], g.var = 0.07, s.int = POCS_s$s.int[1], s.slp = POCS_s$s.slp[1])
MP_POSP <- list(rec = totrecruits_POSP, g.int = POSP_g$g.int[1], g.slp = POSP_g$g.slp[1], g.var = 0.07, s.int = POSP_s$s.int[1], s.slp = POSP_s$s.slp[1])
MP_MOSP <- list(rec = totrecruits_MOSP, g.int = MOSP_g$g.int[1], g.slp = MOSP_g$g.slp[1], g.var = 0.07, s.int = MOSP_s$s.int[1], s.slp = MOSP_s$s.slp[1])
...
bin_size <- min.size + c(0:50) * (max.size - min.size)/50
y <- 0.5 * (bin_size[1:50]+bin_size[2:(50+1)])
I <- y >= rec.size
delta_size <- y[2] - y[1]
...
reproduction_function <- function(y, x, rec, rec.size=-2.5,ind = TRUE) {
  ...
  if (ind == TRUE){
    out <- 10^x
    out[1:3] <- 10^x[1:3] * rec}
    #out[4:length(x)] <- 10^x[4:length(x)]}
  if (ind == FALSE){
    out <-  (10^x) * rec
