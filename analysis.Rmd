...
modS_site_POCS <- glm(survival ~ size + Site, family = "binomial" ,
           data = ColonyLevel[ColonyLevel$Genus_Code == GeneraNames[1],])
...
recruit_site <- matrix(NaN, nrow = length(IntervalYears), ncol = length(SiteNames))
for (i in 1:length(SiteNames)){
  ...
  rec <- numbrecruits * timeint_all # this is the number multiplied by the total area at each time step to give the number of recurits at the following time step.
  
  recruit_site[(1:length(rec)),i] <- rec
}
View(recruit_site)

# sum number of recruits/interval
...
plot(ln_ESc ~ ln_SS ,data = Growth, main="Full growth fit")
...
plot(ln_ESc ~ ln_SS ,data = Growth, main="Genera growth fit at all sites+ all interval years")
...
par(mfrow=c(4,3))
for (h in 1:LoopLeng) {
  plot(ln_ESc ~ ln_SS ,data = Growth, main="Each site growth fit for all genera + all interval years")
